...
def test_medium_positive():
    text = (
        "It's great to see a news outlet that is fair and balanced in its reporting. I appreciate that this outlet "
...
def test_medium_negative():
    text = (
        "Today, I read an article on local news that I found to be extremely disappointing. The article in question"
...
def test_long_positive():
    text = (
        "If you're in the mood for some truly authentic Italian cuisine, then you need to head to Mario's "
...
@pytest.mark.xfail(
    reason="Response should be negative", raises="Assertion Error", run=False
)
def test_long_negative():
    text = (
        "It's been a long time since I've had to write one of these, but here goes. I've been a client of this "
